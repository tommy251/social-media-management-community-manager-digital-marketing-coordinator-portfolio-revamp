(()=>{
    ...
    (()=>{
        ...
        }, "u" > typeof Symbol && (ce[Symbol.iterator] = function() {
            var e = this;
            return {
                next: function() {
                    var t = e.getToken();
        ...
        function pathe_M_eThtNZ_cwd() {
            return "u" > typeof process && "function" == typeof process.cwd ? process.cwd().replace(/\\/g, "/") : "/";
        }
        ...
        }, TypeError), Te.ERR_INVALID_MODULE_SPECIFIER = createError("ERR_INVALID_MODULE_SPECIFIER", (e, t, i)=>`Invalid module "${e}" ${t}${i ? ` imported from ${i}` : ""}`, TypeError), Te.ERR_INVALID_PACKAGE_CONFIG = createError("ERR_INVALID_PACKAGE_CONFIG", (e, t, i)=>`Invalid package config ${e}${t ? ` while importing ${t}` : ""}${i ? `. ${i}` : ""}`, Error), Te.ERR_INVALID_PACKAGE_TARGET = createError("ERR_INVALID_PACKAGE_TARGET", (e, t, i, s = !1, r)=>{
        ...
        const Le = function(e) {
            ...
            return t && (Ne = Error.stackTraceLimit, Error.stackTraceLimit = 1 / 0), Error.captureStackTrace(e), t && (Error.stackTraceLimit = Ne), e;
        });
        ...
        function _resolve(e, t = {}) {
            ...
            let n;
            for (const s of r){
                if (n = _tryModuleResolve(e, s, i)) break;
                for (const r of [
                    "",
                    "/index"
                ]){
                    for (const a of t.extensions || nt)if (n = _tryModuleResolve(dist_joinURL(e, r) + a, s, i)) break;
                    if (n) break;
                }
                if (n) break;
            }
        ...
        const _t = globalThis.process?.platform || "", bt = (std_env_dist_n(gt.CI) || yt.ci, std_env_dist_n(globalThis.process?.stdout && globalThis.process?.stdout.isTTY)), kt = (std_env_dist_n(gt.DEBUG), "test" === xt || std_env_dist_n(gt.TEST), std_env_dist_n(gt.MINIMAL), /^win/i.test(_t)), wt = (/^linux/i.test(_t), /^darwin/i.test(_t), !std_env_dist_n(gt.NO_COLOR) && (std_env_dist_n(gt.FORCE_COLOR) || (bt || kt) && gt.TERM), (globalThis.process?.versions?.node || "").replace(/^v/, "") || null), It = (Number(wt?.split(".")[0]), globalThis.process || Object.create(null)), Ct = {
            versions: {}
        }, Rt = (new Proxy(It, {
        ...
        const Lt = require("node:tty"), Ot = Lt?.WriteStream?.prototype?.hasColors?.() ?? !1, base_format = (e, t)=>{
            if (!Ot) return (e)=>e;
            const i = `[${e}m`, s = `[${t}m`;
            return (e)=>{
                const r = e + "";
        ...
        function jitiResolve(e, t, i) {
            ...
            for (const r of e.additionalExts){
                if (s = tryNativeRequireResolve(e, t + r, n, i) || tryNativeRequireResolve(e, t + "/index" + r, n, i)) return s;
                if ((Wt.test(e.filename) || Wt.test(e.parentModule?.filename || "") || qt.test(t)) && (s = tryNativeRequireResolve(e, t.replace(qt, ".$1t$2"), n, i))) return s;
            }
            if (!i?.try) throw r;
        }
